...
class MSAFBlock(nn.Module):
    def __init__(self, in_channels, block_channel, block_dropout=0., lowest_atten=0., reduction_factor=4):
        ...
        self.joint_features = nn.Sequential(
            ...
            nn.Linear(self.reduced_channel, self.reduced_channel),
            nn.BatchNorm1d(self.reduced_channel),
            nn.ReLU(inplace=True)
        )
...
#     m1 = torch.rand(4, 8, 32, 32).to(device)
#     m2 = torch.rand(4, 8, 32, 32).to(device)

#     print(f"Size of input embeddings: {m1.shape, m2.shape}")
#     x = [m1, m2]
#     net = MSAF([8, 8], 8, block_dropout=0.2, reduction_factor=4, split_block=5).to(device)
#     y = net(x)
#     print(f"Size of output embeddings: {y[0].shape, y[1].shape}")
#     combined_embedding = torch.cat(y, dim=1)
#     print(f"Size of combined embeddings: {combined_embedding.shape}")
#     combined_embedding = combined_embedding.view(combined_embedding.size(0), -1)
#     print(f"Size of flattened combined embeddings: {combined_embedding.shape}")